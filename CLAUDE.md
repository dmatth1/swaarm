...
- **Invocation timeout**: `run_claude()` wraps `claude` with `timeout $CLAUDE_TIMEOUT` (default 1800s / 30m). Kills hung invocations (e.g. deadlocked test binaries). Logs `[timeout]` on kill. Skipped on macOS (no GNU timeout); active in Docker.
