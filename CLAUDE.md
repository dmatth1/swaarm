...
**Phase 3 — Monitoring**: Claude Code (you) monitors via `/loop`, checking `docker ps` + git task state every 5m. Reviews completions, runs specialist sweeps periodically, handles failures adaptively.
...
- **Remote repo mirroring**: background mirror loop on host pushes to GitHub every 30s. `PUBLIC_REPO=true` triggers security notice.
